...
int main(int argc, char **argv) {
	...
	for (int y = 0; y < h; ++y) {
		for (int x = 0; x < w; ++x) {
			const int idx = y*w + x;
			const int idx_flip = y*w + (w-1 - x); 
			rotated[idx_flip*3 + 0] = img[idx*3 + 0];
			rotated[idx_flip*3 + 1] = img[idx*3 + 1];
			rotated[idx_flip*3 + 2] = img[idx*3 + 2];
		}
	}
	stbi_image_free(img);

	nob_log(NOB_INFO, "Writing image file!");
	stbi_write_jpg("output.jpg", w, h, 3, rotated, 3*w);
	free(rotated);
}
